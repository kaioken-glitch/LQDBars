...
import { FaSearch, FaPlay, FaDownload, FaHeart, FaThumbsDown, FaMinus } from 'react-icons/fa';
...
export default function Favorites() {
  ...
  const isDownloaded = (song) => downloadedSongs.some(s => s.id === song.id);

  function toggleDownloaded(song) {
    const exists = isDownloaded(song);
    if (exists) {
      const next = downloadedSongs.filter(s => s.id !== song.id);
      setDownloadedSongs(next);
      localStorage.setItem('downloadedSongs', JSON.stringify(next));
    } else {
      const next = [...downloadedSongs, { ...song, downloaded: true }];
      setDownloadedSongs(next);
      localStorage.setItem('downloadedSongs', JSON.stringify(next));
    }
  }
  ...
  function removeFromFavorites(songId) {
    const nextDownloaded = downloadedSongs.filter(s => s.id !== songId);
    setDownloadedSongs(nextDownloaded);
    localStorage.setItem('downloadedSongs', JSON.stringify(nextDownloaded));
    setSongFlags(prev => {
      const next = { ...prev };
      if (next[songId]) next[songId].favorite = false;
      try { localStorage.setItem('lb:songFlags', JSON.stringify(next)); } catch (e) {}
      return next;
    });
  }
  ...
  const likedSongs = useMemo(() => library.filter(s => s.like || s.liked), [library]);

  const favoriteAlbums = useMemo(() => {
    const fav = library.filter(s => s.favorite);
    const groups = {};
    fav.forEach(s => {
    ...
    return Object.keys(groups).map(k => ({ album: k, songs: groups[k] }));
  }, [library]);

  function playSongFromAlbum(song, albumSongs) {
  ...
  return (
    <div className="favorites flex flex-col items-center justify-start w-full h-full pb-20">
      {/* Header Section */}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="mt-4">
            ...
            <p className="text-white/60 text-sm md:text-base mt-2">
              {favoriteAlbums.length + (likedSongs.length > 0 ? 1 : 0)} collections • {library.filter(s => s.favorite || s.like || s.liked).length} songs
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="w-full max-w-[1600px] px-4 md:px-6 lg:px-8 mt-6 flex-1 overflow-y-auto">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6 pb-8">
          ...
          {likedSongs.length > 0 && (
            <div 
              onClick={() => openAlbum({ album: 'Liked Songs', songs: likedSongs })} 
            ...
            >
              {/* Album Grid */}
              <div className="aspect-square bg-gradient-to-br from-pink-500/40 to-purple-600/40 
              rounded-t-2xl grid grid-cols-2 grid-rows-2 gap-0.5 overflow-hidden p-0.5">
                {likedSongs.slice(0, 4).map((s, idx) => (
                  <img 
                    key={idx} 
                    src={s.cover} 
                    alt={s.name} 
              ...
              <div className="p-4">
                ...
                <p className="text-white/60 text-xs mb-3">{likedSongs.length} song{likedSongs.length !== 1 ? 's' : ''}</p>
                
                {/* Play Button */}
                <button 
                  onClick={(e) => { 
                    e.stopPropagation(); 
                    setPlayerSongs(likedSongs); 
                    setCurrentIndex(0); 
                    setIsPlaying(true); 
                  }} 
          ...
          {favoriteAlbums.map((alb) => (
            ...
            >
              ...
              <div className="p-4">
                ...
                <p className="text-white/60 text-xs mb-3 truncate">{alb.songs[0]?.artist || ''}</p>
                  ...
                  <div key={song.id || index}>
                    ...
                    <div className="block md:hidden">
                      <SongTile
                        ...
                        onToggleDownload={() => toggleDownloaded(song)}
                      />
                    </div>

                    {/* Desktop View */}
                    <div className="hidden md:flex songItem items-center gap-4 px-4 py-3 rounded-xl 
                    ...
                    hover:border-white/10">
                      ...
                      <div className="flex items-center gap-1">
                        <button 
                          title="Downloaded" 
                          onClick={() => toggleDownloaded(song)} 
                          className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-white/10 
                          transition-all"
                        >
                          <FaDownload className={`text-sm ${isDownloaded(song) ? 'text-emerald-400' : 'text-white/40 group-hover:text-white/60'}`} />
                        </button>
                        <button 
                          title="Favorite" 
                          onClick={() => toggleFlag(song.id, 'favorite')} 
                          className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-white/10 
                          transition-all"
                        >
                          <FaHeart className={`text-sm ${songFlags[song.id]?.favorite ? 'text-yellow-400' : 'text-white/40 group-hover:text-white/60'}`} />
                        </button>
                        <button 
                          title="Dislike" 
                          onClick={() => toggleFlag(song.id, 'dislike')} 
                          className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-white/10 
                          transition-all"
                        >
                          <FaThumbsDown className={`text-sm ${songFlags[song.id]?.dislike ? 'text-red-400' : 'text-white/40 group-hover:text-white/60'}`} />
                        </button>
                        <button 
                          title="Remove" 
                          ...
                          transition-all"
                        >
                          <FaMinus className="text-sm text-white/40 group-hover:text-white/60" />
                        </button>
                        <button 
                          title="Play" 
