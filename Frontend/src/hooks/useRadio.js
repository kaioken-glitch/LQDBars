...
export function useRadio() {
  const {
    ...
    currentIndex,
    songs,
    setSongs,       // expose this in PlayerContext if not already
  } = usePlayer();
  ...
  }, [currentSong, currentIndex, songs, setSongs]);

  /* ── Stop radio — clear radio songs from queue ── */
  const stopRadio = useCallback(() => {
    setSongs(prev => prev.filter(s => s.source !== 'radio'));
    setRadioMode(false);
    setRadioError(null);
  ...
  }, [radioMode, radioLoading, currentIndex, songs, setSongs]);
