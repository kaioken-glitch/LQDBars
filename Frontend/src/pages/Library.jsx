...
/* ══ GRID TILES — art-first, text lives below the artwork instead of
   inside a dark card box; matches Playlists.jsx's canonical tile. ══ */
.lib-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 22px 18px; }
@media (min-width: 500px)  { .lib-grid { grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 24px 20px; } }
@media (min-width: 768px)  { .lib-grid { grid-template-columns: repeat(auto-fill, minmax(172px, 1fr)); gap: 26px 22px; } }
@media (min-width: 1024px) { .lib-grid { grid-template-columns: repeat(auto-fill, minmax(184px, 1fr)); gap: 28px 24px; } }

.lib-card { position: relative; cursor: pointer; animation: libUp .38s var(--spring) both; }
@keyframes libUp { from { opacity: 0; transform: translateY(14px); } to { opacity: 1; transform: none; } }
...
.lib-art { position: relative; width: 100%; aspect-ratio: 1 / 1; border-radius: 14px; overflow: hidden; background: var(--gdim); box-shadow: 0 10px 26px rgba(0,0,0,.4); transition: box-shadow .22s var(--ease); }
.lib-card:hover .lib-art { box-shadow: 0 16px 40px rgba(0,0,0,.55), 0 0 0 1px rgba(255,255,255,.06); }
.lib-art-mosaic { position: absolute; inset: 0; display: grid; grid-template-columns: 1fr 1fr; gap: 1px; }
.lib-art-single { position: absolute; inset: 0; overflow: hidden; }
.lib-art-mosaic img, .lib-art-single img { width: 100%; height: 100%; object-fit: cover; display: block; transition: transform .5s var(--ease); }
.lib-card:hover .lib-art-mosaic img, .lib-card:hover .lib-art-single img { transform: scale(1.06); }
.lib-art-overlay { position: absolute; inset: 0; background: linear-gradient(to top, rgba(0,0,0,.35) 0%, transparent 45%); opacity: 0; transition: opacity .2s var(--ease); }
.lib-card:hover .lib-art-overlay { opacity: 1; }

/* Small hover play button, bottom-right — no big centered glow ring */
.lib-play-btn { position: absolute; right: 8px; bottom: 8px; z-index: 3; width: 34px; height: 34px; border-radius: 50%; background: #fff; color: #000; font-size: 12px; display: flex; align-items: center; justify-content: center; box-shadow: 0 6px 18px rgba(0,0,0,.5); opacity: 0; transform: translateY(4px) scale(.9); transition: opacity .18s var(--ease), transform .18s var(--spring), background .15s; }
.lib-card:hover .lib-play-btn { opacity: 1; transform: translateY(0) scale(1); }
.lib-play-btn:hover { background: var(--g); color: #000; }

/* Text below the art, plain — no card background/border */
.lib-card-info { padding: 10px 1px 0; }
.lib-card-name { font-family: 'Syne', sans-serif; font-size: 13.5px; font-weight: 700; color: var(--t1); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; line-height: 1.3; letter-spacing: -.01em; }
.lib-card-artist { font-size: 11.5px; color: var(--t2); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-top: 2px; }
.lib-card-count { font-size: 11.5px; color: var(--t3); margin-top: 2px; }
